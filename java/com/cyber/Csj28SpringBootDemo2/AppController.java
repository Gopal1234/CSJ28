...
public class AppController {
	...
	@RequestMapping(path="/checkFlights")
	public ModelAndView checkFlight(@RequestParam("t1") String source, @RequestParam("t2") String destination, @RequestParam("t3") String doj)
	{
		LocalDate date=LocalDate.parse(doj, DateTimeFormatter.ofPattern("yyyy-MM-dd"));
         		List<Flight> flights=dao.getSearchedFlight(source,destination,date);
         		ModelAndView mv=new ModelAndView();
         		mv.addObject("searchFlight",flights);
    	        mv.setViewName("goBookFlight");
    	        return mv;
